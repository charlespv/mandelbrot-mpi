...
int main(int argc, char *argv[]) {
  ...
  /* Allocation memoire du tableau resultat */ 
	...
	{
		pima = ima = (unsigned char *)malloc( w*(h/size)*sizeof(unsigned char));
	}
printf("2\n");
  if( ima == NULL) {
  ...
  /*y = ymin+ (h/size)*my_rank; // fixme*/
  for (i = 0; i < (h/size); i++) {
    ligne = (h*my_rank/size)+i;
    y = ymin + ligne*yinc;
    /*x = xmin;*/
    for (j = 0; j < w; j++) {
      // printf("%d\n", xy2color( x, y, prof));
      ...
      *pima = xy2color( x, y, prof); 
      /*x += xinc;*/
		...
		/* Changer le valeur de y en parallèle */
    /*y += yinc;*/ 
  }
  printf("3\n");
	...
	if(my_rank != 0)
	{
		MPI_Send(ima,(h)*w,MPI_CHAR,0,99,MPI_COMM_WORLD);
	}
	else
	{
		for(i=1;i<size;i++)
		{
			MPI_Recv(ima,(h/size)*w,MPI_CHAR,i,99,MPI_COMM_WORLD,&status);
		}
	}
  printf("Rank:%d -- 4\n", my_rank);
  ...
  /* Sauvegarde de la grille dans le fichier resultat "mandel.ras" */
	...
	{
		printf("entre \n");
	  sauver_rasterfile( "tp2.ras", w, h, ima);
		printf("sortie \n");
	}
printf("5\n");
	/* Finaliser MPI */
